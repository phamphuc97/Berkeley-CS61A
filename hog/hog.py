...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    one_was_rolled = False
    sum_of_rolls, counter = 0, 0
    ...
    while counter < num_rolls:
        ...
        sum_of_rolls += result
        counter += 1
    return sum_of_rolls if not one_was_rolled else 1
...
def picky_piggy(score):
    ...
    """
    # BEGIN PROBLEM 2
    exp = 6 if score % 6 == 0 else score % 6
    if score == 0:
        return 7
    else:
        return int(1 / 7 * 10 ** exp) % 10
    # END PROBLEM 2
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # BEGIN PROBLEM 5
    goal_is_achieved = False
    while not goal_is_achieved:
        num_rolls = strategy0(score0, score1) if who == 0 else strategy1(score1, score0)
        opponent_score = score1 if who == 0 else score0
        score_for_turn = take_turn(num_rolls, opponent_score, dice, goal)
        ...
        goal_is_achieved = (score0 >= goal) or (score1 >= goal)
        who = next_player(who)
    # END PROBLEM 5
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if gain > running_high:
            new_high = gain
            print(str(new_high) + " point(s)! That's a record gain for Player " + player + "!")
        else:
            new_high = running_high
        return announce_highest(who, current_score, new_high)
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    counter = 1
    num_dice_of_max_avg = -1
    max_average = -1
    avg_score_for_rolling_n_dice = make_averaged(roll_dice, trials_count)

    while counter <= 10:
        avg_score_for_n_rolls = avg_score_for_rolling_n_dice(counter, dice)
        if avg_score_for_n_rolls > max_average:
            num_dice_of_max_avg = counter
            max_average = avg_score_for_n_rolls
        counter += 1
    return num_dice_of_max_avg
    # END PROBLEM 9
...
def picky_piggy_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    return 0 if picky_piggy_score >= cutoff else num_rolls
    # END PROBLEM 10
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    picky_piggy_score = picky_piggy(opponent_score)
    if hog_pile(score + picky_piggy_score, opponent_score) or picky_piggy_score >= cutoff:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
