...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    valid_paragraphs = []
    valid_paragraphs += [paragraph for paragraph in paragraphs if select(paragraph)]
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        return typed_word
    lowest_difference, smallest_different_word = diff_function(typed_word, valid_words[0], limit), valid_words[0]
    for word in valid_words[1:]:
        if diff_function(typed_word, word, limit) < lowest_difference:
            lowest_difference = diff_function(typed_word, word, limit)
            smallest_different_word = word
    if lowest_difference <= limit:
        return smallest_different_word
    else:
        return typed_word
    # END PROBLEM 5
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    def progress(sofar, prompt):
        if sofar == [] or prompt == [] or sofar[0] != prompt[0]:
            return 0
        return 1 + progress(sofar[1:], prompt[1:])
    progress = progress(sofar, prompt) / len(prompt)
...
def time_per_word(words, times_per_player):
    ...
    for time in times_per_player:
        ...
        while count < len(time) - 1:
            new_lst.append(time[count + 1] - time[count])
            count += 1
        difference.append(new_lst)
    return match(words, difference)
...
def fastest_words(match):
    ...
    #        record = [time(match, i, word) for i in player_indices]
    #        if min(record) == time(match, player, word) and time(match, player, word) not in record[0:player]:
    #            words_per_player.append(word_at(match, word))
    #    fastest_lst.append(words_per_player)
    
    fastest_lst = [[] for i in range(len(player_indices))]
    for player in player_indices:
        for word in word_indices:
            record = [time(match, i, word) for i in player_indices]
            if min(record) == time(match, player, word) and time(match, player, word) not in record[0:player]:
                fastest_lst[player].append(word_at(match, word))
