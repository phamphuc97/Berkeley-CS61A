...
def group_by_centroid(restaurants, centroids):
    ...
    #for r in restaurants:
        #closest_centroid = find_closest(restaurant_location(r), centroids)
        #closest_pairs += [[closest_centroid, r]]
    closest_pairs = [[find_closest(restaurant_location(r), centroids), r] for r in restaurants]
    return group_by_first(closest_pairs)
    # END Question 4
...
def find_predictor(user, restaurants, feature_fn):
    ...
    S_xx = sum([i ** 2 for i in xx])
    S_yy = sum([i ** 2 for i in yy])
    S_xy = sum([xx[i] * yy[i] for i in range(0, len(xx))])
    b = S_xy / S_xx
    a = mean(ys) - b * mean(xs)
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    predictor_lst = [list(find_predictor(user, reviewed, feature_fn)) for feature_fn in feature_fns]
    return max(predictor_lst, key=lambda x: x[1])[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for r in restaurants:
        if r in reviewed:
            rating[restaurant_name(r)] = user_score(user, restaurant_name(r))
        else:
